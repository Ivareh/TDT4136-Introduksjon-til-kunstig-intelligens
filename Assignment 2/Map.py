...
class Map_Obj():
    ...
    def __init__(self, task: int = 1) -> None:
        ...
        """
        print('Creating map for task ' + str(task) + '...')
        ...
        self.edges: dict[tuple([int, int]), list[tuple([int, int])]] = self.set_edges(self.int_map)
        self.tick_counter = 0
        print("Start position: " + str(self.get_start_pos()))
        print("End goal position: " + str(self.get_end_goal_pos()))
    ...
    def show_map(self, themap: Union[np.ndarray, str] = None):
        ...
        colors = {
            ...
            ' S ': (255, 0, 255),  # magenta
            ' G ': (0, 128, 255),   # cyan
            ' P ': (66, 255, 80),  # green, indicating path taken
        }
    ...
    def set_edges(self, int_map) -> dict[tuple([int, int]), list[tuple([int, int])]]:
        """
        Sets the edges of the map. The edges are defined as the
        positions in the map that are not walls.
        """
        edges: dict[tuple([int, int]), list[tuple([int, int])]] = {}
        for i in range(int_map.shape[0]):
            for j in range(int_map.shape[1]):
                if int_map[i][j] != -1:
                    edges[(i, j)] = []
                    if int_map[i-1][j] != -1:
                        edges[(i, j)].append(tuple([i-1, j]))
                    if int_map[i+1][j] != -1:
                        edges[(i, j)].append(tuple([i+1, j]))
                    if int_map[i][j-1] != -1:
                        edges[(i, j)].append(tuple([i, j-1]))
                    if int_map[i][j+1] != -1:
                        edges[(i, j)].append(tuple([i, j+1]))
        return edges
