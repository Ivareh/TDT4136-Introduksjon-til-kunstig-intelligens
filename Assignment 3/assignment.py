...
class CSP:
    def __init__(self):
        ...
        self.backtrack_F_count = 0
        self.backtrack_F_failure_count = 0
    ...
    def backtrack(self, assignment):
        ...
        """
        # TODO: YOUR CODE HERE
        self.backtrack_F_count += 1

        var = self.select_unassigned_variable(assignment)


        if not var: # if all variables are assigned
            return assignment 
        
        for value in assignment[var]:
            nassignment = copy.deepcopy(assignment) # make a deep copy of the assignment
            nassignment[var] = list(value) # assign the value to the variable

            inferences = self.inference(nassignment, self.get_all_neighboring_arcs(var))
            if inferences: # if the inference is not empty
                result = self.backtrack(nassignment)
                if result is not None:
                    return result
        
        self.backtrack_F_failure_count += 1
        return None
    ...
    def inference(self, assignment, queue):
        ...
        while queue: 
            Xi, Xj = queue.pop(0)
            if self.revise(assignment, Xi, Xj): # if the domain is revised
                if len(assignment[Xi]) == 0: # if the domain is empty
                    return False
    ...
    def revise(self, assignment, i, j):
        ...
        # TODO: YOUR CODE HERE
        revised = False
        for domainV in assignment[i]:
            domainVSDone = False # if the domain value 1 search is done
            for domainV2 in assignment[j]:
                if (domainV, domainV2) in self.constraints[i][j]: # if the value is in the constraints
                    domainVSDone = True
            if not domainVSDone: # if the domain value 1 search is not done
                assignment[i].remove(domainV)
                revised = True
        return revised
...
if __name__ == "__main__":
    ...
    # Solve all of the Sudoku boards
    sudokus = [
        'easy.txt',
        'medium.txt',
        'hard.txt',
    ...
    for sudoku in sudokus:
        ...
        print('===============================')
        print()
        print(f'Backtrack F count: {csp.backtrack_F_count}')
        print(f'Backtrack F failure count: {csp.backtrack_F_failure_count}')
        print()
